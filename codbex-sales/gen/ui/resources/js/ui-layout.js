	...
	this.init = function(containerEl, viewNames, id, reconstruct){
		...
		if(id){
			if(!reconstruct){
				...
				var savedState = localStorage.getItem('DIRIGIBLE.application.codbex-sales.'+ id);
				if(savedState !== null) {
		...
		let focusView = localStorage.getItem("DIRIGIBLE.application.codbex-sales.focusView");
		...
		if(id){
			this.layout.on('stateChanged', function(){
				...
				localStorage.setItem('DIRIGIBLE.application.codbex-sales.'+ id, state );
			}.bind(this));			
		}
