...
exports.count = function (Customer) {
	let resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "CODBEX_CONTACT" WHERE "CONTACT_CUSTOMERID" = ?', [Customer]);
...
exports.customDataCount = function() {
	let resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "CODBEX_CONTACT"');
	if (resultSet !== null && resultSet[0] !== null) {
		if (resultSet[0].COUNT !== undefined && resultSet[0].COUNT !== null) {
			return resultSet[0].COUNT;
		} else if (resultSet[0].count !== undefined && resultSet[0].count !== null) {
