...
const dao = require("hermes-app/gen/dao/Leads/LeadNote");
...
rs.service()
	...
	.resource("count/{Lead}")
		.get(function(ctx, request) {
			let Lead = parseInt(ctx.pathParameters.Lead);
			Lead = isNaN(Lead) ? ctx.pathParameters.Lead : Lead;
			http.sendResponseOk("" + dao.count(Lead));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("LeadNote not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/hermes-app/gen/api/LeadNote.js/" + entity.Id);
