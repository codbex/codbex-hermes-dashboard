...
const dao = require("hermes-app/gen/dao/Quotations/QuotationItem");
...
rs.service()
	...
	.resource("count/{Quotation}")
		.get(function(ctx, request) {
			let Quotation = parseInt(ctx.pathParameters.Quotation);
			Quotation = isNaN(Quotation) ? ctx.pathParameters.Quotation : Quotation;
			http.sendResponseOk("" + dao.count(Quotation));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("QuotationItem not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/hermes-app/gen/api/QuotationItem.js/" + entity.Id);
