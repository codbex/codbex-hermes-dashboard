...
const dao = require("hermes-app/gen/dao/Opportunities/OpportunityNote");
...
rs.service()
	...
	.resource("count/{Opportunity}")
		.get(function(ctx, request) {
			let Opportunity = parseInt(ctx.pathParameters.Opportunity);
			Opportunity = isNaN(Opportunity) ? ctx.pathParameters.Opportunity : Opportunity;
			http.sendResponseOk("" + dao.count(Opportunity));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("OpportunityNote not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/hermes-app/gen/api/OpportunityNote.js/" + entity.Id);
